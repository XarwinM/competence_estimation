...
class PfndELU(t.nn.Module):
    def __init__(self, dim=1, ls=16, n_condim=4):
        ...
        if n_condim > 0:
            self.con = True
            self.subnet = t.nn.Sequential(
                t.nn.Linear(n_condim, 50),
                t.nn.ReLU(),
                t.nn.Linear(50, 50),
                t.nn.ReLU(),
    ...
    def function(self, x):
        xn = x.unsqueeze(2)
        if self.con:
            return t.exp(self.alpha) * (
                x
                + 0.8
                * self.sig(self.eps)
                * t.sum(
                    self.actfunc(xn * self.mat1 + self.bias1)
                    * self.mat2
                    / (
                        t.sum(t.nn.ReLU()(-self.mat1 * self.mat2), dim=2).unsqueeze(1)
                        + 1
                    ),
                    dim=2,
                )
            ...
            return t.exp(self.alpha) * (
                ...
                * t.sum(
                    ...
                    * self.mat2
                    / (
                        t.sum(t.nn.ReLU()(-self.mat1 * self.mat2), dim=1).unsqueeze(1)
                        + 1
                    ),
                    dim=2,
                )
        ...
        if self.con:
            return (
                ...
                * (
                    t.sum(
                        0.8
                        * self.actderiv(xn * self.mat1 + self.bias1)
                        * self.mat2
                        * self.mat1
                        / (
                            t.sum(t.nn.ReLU()(-self.mat1 * self.mat2), dim=2).unsqueeze(
                                1
                            )
                            + 1
                        ),
        ...
        if self.con:
            return t.exp(self.alpha) * (
                ...
                * t.sum(
                    self.actfunc(xn * self.mat1 + self.bias1)
                    * self.mat2
                    / (t.sum(t.abs(self.mat1 * self.mat2), dim=2).unsqueeze(1) + 1),
                    dim=2,
                )
                + self.bias2
            )
            ...
            return t.exp(self.alpha) * (
                ...
                * t.sum(
                    self.actfunc(xn * self.mat1 + self.bias1)
                    * self.mat2
                    / (t.sum(t.abs(self.mat1 * self.mat2), dim=1).unsqueeze(1) + 1),
                    dim=2,
                )
                + self.bias2
            )
