...
env_dictionary = {
    "PACS": {0: "Art", 1: "Cartoon", 2: "Photo", 3: "Sketch"},
...
def plot_accuracy_coverage(
    ...
    if axs is None:
        ...
        assert num_rows == len(domains) or num_cols == len(
            domains
        ), "Number of axes must match number of domains"
    ...
    for i_domain, domain in enumerate(domains):
        ...
        for score in score_fcts:
            ...
            quantiles_coverages = []
            quantiles_coverages_x = (
                []
            )  # Quantile w.r.t. fraction of data in competence region
            acc_ids, acc_oods = [], []
            ...
            for classifier in classifiers:
                ...
                scores_ood = np.array(
                    results_scores[score][data_set][classifier][domain][percentages][
                        "ood_test"
                    ]
                )
                y_true_ood = np.array(
                    results_true_false[score][data_set][classifier][domain][
                        percentages
                    ]["ood_test"]
                )
                scores_id = np.array(
                ...
                for i in range(5, len(scores_ood), 5):
                    ...
                    accuracy = (
                        y_true_ood[scores_ood[i] >= scores_ood].sum()
                        / (scores_ood[i] >= scores_ood).sum()
                    )
                    ...
                    if len(quantiles) > 0:
                        if (
                            scores_ood[i] > quantiles[0]
                            and scores_ood[i - 1] >= quantiles[0]
                        ):
            ...
            # for score in score_fcts:
            axs[i_domain].scatter(
                quantile_means_x[score],
                quantile_means_y[score],
                s=65,
                color=colors[score],
                marker="o",
                label=f"95%-Percentile ({score})",
            )
...
def plot_accuracy_curve(
    ...
    if axarr is None:
        ...
        assert num_rows == len(score_functions) or num_cols == len(
            score_functions
        ), "Number of axes must match number of score functions"

    # Loop through scores for given data set, classifier and domain
    for i, (score, ax) in enumerate(
        zip(score_functions, axarr.flat[: len(score_functions)])
    ):
        ...
        # Add 95% quantile
        score_val = (
            results_scores[score][data_set][classifier][domain][0.0]["id_val"] - mean
        ) / std
        ...
        for code_s, code_a in zip(color_codes_scores.keys(), color_codes_accs.keys()):
            # Normalize scores
            score_tests = results_scores[score][data_set][classifier][domain][0.0][
                code_s
            ]
            ...
            # Plot accuracies
            if code_a == "ood_test":
                res = results_true_false[score][data_set][classifier][domain][0.0][
                    code_a
                ]
                cum_accuracy = np.cumsum(res[sort_idx]) / np.arange(1, len(res) + 1)
                ax.plot(
                    x[show_idx],
                    cum_accuracy[show_idx],
                    linestyle="solid",
                    color=color_codes_accs[code_s],
                    alpha=0.6,
                    lw=3,
                    label=label_acc,
                )
                ...
                ax.plot(
                    x[show_idx],
                    y[show_idx],
                    color=color_codes_scores[code_s],
                    alpha=0.9,
                    lw=2.8,
                    linestyle="dotted",
                    label=label_frac,
        ...
        # Accuracy ood
        acc_od = results_true_false[score][data_set][classifier][domain][0.0][
            "ood_test"
        ]
        ...
        if i == 0:
            ax.set_ylabel(
                f"{data_set} ({env_dictionary[data_set][domain]})",
                fontsize=18,
                labelpad=10,
            )
        else:
